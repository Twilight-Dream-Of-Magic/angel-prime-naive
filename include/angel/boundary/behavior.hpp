...
namespace angel::boundary::behavior {

template<std::size_t StateCount, std::size_t AlphabetSize>
struct MooreMachine final {
...
struct BehaviorPartition final {
...
template<std::size_t StateCount, std::size_t AlphabetSize>
[[nodiscard]] inline BehaviorPartition<StateCount> minimal_partition(
    const MooreMachine<StateCount, AlphabetSize>& machine) {
    if (!machine.valid()) throw std::invalid_argument("invalid finite causal machine");

    BehaviorPartition<StateCount> partition{};
    std::vector<std::uint64_t> observations;
    observations.reserve(StateCount);
    for (std::size_t state = 0U; state < StateCount; ++state) {
        const auto it = std::find(
            observations.begin(), observations.end(), machine.observation[state]);
...
template<std::size_t StateCount, std::size_t AlphabetSize>
[[nodiscard]] inline bool observation_is_resumable(
    const MooreMachine<StateCount, AlphabetSize>& machine) {
    const auto partition = minimal_partition(machine);
...
struct StabilityCertificate final {
...
template<std::size_t StateCount, std::size_t AlphabetSize>
[[nodiscard]] inline std::optional<StabilityCertificate> certify_stability(
    const MooreMachine<StateCount, AlphabetSize>& machine,
    const std::uint16_t start) {
